...
export type CalloutsConfigFull = {
    note: CalloutConfig,
    tip: CalloutConfig,
    warning: CalloutConfig,
    abstract: CalloutConfig,
    info: CalloutConfig,
    todo: CalloutConfig,
    success: CalloutConfig,
    question: CalloutConfig,
    danger: CalloutConfig,
    bug: CalloutConfig,
    example: CalloutConfig,
    failure: CalloutConfig,
    quote: CalloutConfig
};

export type ConfigFull = {
    callouts: CalloutsConfigFull,
    calloutContainer: string,
    embedDefaultColor: boolean,
    customClassNames: string[]
};

//export type CalloutsConfig = Partial<CalloutsConfigFull>;
export type CalloutsConfig = {
    [K in keyof CalloutsConfigFull]?: Partial<CalloutConfig>
};

export type Config = Omit<Partial<ConfigFull>, "callouts"> & {
    callouts?: CalloutsConfig
};

export const defaultConfig: ConfigFull = {
    // copied from 
    ...
    calloutContainer: "blockquote",
    customClassNames: [],
    embedDefaultColor: false
};


export function determineCalloutType(type: string) {
    return defaultConfig.callouts?.hasOwnProperty(type)
        ? type
        : calloutTypeMap.get(type) ?? "note";
};

export function mergeWithDefault(config: Config): ConfigFull {
    return mergeDeep(config, defaultConfig) as ConfigFull;
};

export function mergeDeep(obj: any, template: any): any {
    const merged: any = {};
    for (const key in template) {
        if (isObject(template[key])) {
            merged[key] = mergeDeep(obj[key] ?? {}, template[key]);
        } else {
            merged[key] = obj[key] ?? template[key];
        }
    }
    return merged;
}
...
const calloutTypeMap: Map<string, string> = new Map(Object.entries({
    summary: "abstract",
    tldr: "abstract",
    hint: "tip",
    important: "tip",
